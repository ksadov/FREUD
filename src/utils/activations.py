...
def activation_tensor_from_indexed(
    activation_values: torch.Tensor, activation_indices: torch.Tensor, feature_idx: int
) -> torch.Tensor:
    """
    ...
    for i, top_indices_per_file in enumerate(activation_indices):
        ...
        for j, top_indices_per_timestep in enumerate(top_indices_per_file):
            if feature_idx in top_indices_per_timestep:
                index_of_feature = (
                    (top_indices_per_timestep == feature_idx).nonzero().item()
                )
...
@torch.no_grad()
def top_activations(
    dataloader: MemoryMappedActivationDataLoader | FlyActivationDataLoader,
    feature_idx: int,
    n_files: int,
    ...
    list[tuple[str, torch.Tensor, float, float]], list[float], Optional[list[float]]
]:
    """
    ...
    for batch in tqdm(dataloader):
        if dataloader.activation_type == "tensor":
            ...
            act = activation_tensor_from_indexed(act_batch, indexes, feature_idx)
        for audio_file, act in zip(audio_files, act):
            ...
            if absolute_magnitude:
                ...
                signed_max_activation_value = trimmed_activation[
...
def top_activations_for_audio(
    ...
    for top_acts_at_t, top_indices_at_t in zip(top_acts, top_indices):
        unique_top_activations.extend(
            [
                (idx.item(), value.item())
                for idx, value in zip(top_indices_at_t, top_acts_at_t)
            ]
        )
        # sort by value
    ...
    for i, v in unique_top_activations:
        if indexed_activations:
            act = (
                activation_tensor_from_indexed(
        ...
        assert (
            act.max() == v
        ), f"Max activation at index {i} is {act.max()} but expected {v}"
...
def manipulate_latent(
    ...
    whisper_cache: WhisperActivationCache,
    sae_model: Optional[L1AutoEncoder | TopKAutoEncoder],
    whisper_subbed: WhisperSubbedActivation,
    feat_idx: int,
    manipulation_factor: float,
) -> tuple[Optional[str], str, str, torch.Tensor, torch.Tensor]:
    """
    ...
    if sae_model:
        ...
        if isinstance(output.encoded, L1EncoderOutput):
            ...
            manipulated_value = (
                output.encoded.latent[:, :, feat_idx] * manipulation_factor
            )
    ...
    return (
        baseline_text,
        manipulated_subbed_result.text,
        standard_subbed_result.text,
        value_pre_activation_trimmed,
