...
from src.dataset.activations import (
    ...
    init_sae_from_checkpoint,
)
from src.utils.activations import (
    top_activations,
    top_activations_for_audio,
    manipulate_latent,
)
from src.models.hooked_model import (
    init_cache,
...
def get_gui_data(
    config: dict, from_disk: bool, files_to_search: Optional[int]
) -> tuple[
    callable,
    int,
    str,
    ...
    if from_disk:
        ...
        sae_model = (
            init_sae_from_checkpoint(config["sae_model"], config["device"])
            if config["sae_model"] is not None
            else None
        )
    else:
    ...
    return (
        lambda feature_idx, n_files, max_val, min_val, absolute_magnitude, return_max_per_file: top_activations(
            dataloader,
            feature_idx,
            n_files,
            max_val,
...
def init_gui_data(
    config_path: str,
    from_disk: bool,
    files_to_search: Optional[int],
    no_audio_upload: bool,
):
    with open(config_path, "r") as f:
        config = json.load(f)
    (
        GlobalState.top_fn,
        GlobalState.n_features,
        GlobalState.layer_name,
        GlobalState.whisper_cache,
        GlobalState.sae_model,
        GlobalState.whisper_subbed,
    ) = get_gui_data(config, from_disk, files_to_search)
        ...
        return jsonify(
            {
                ...
                "n_features": GlobalState.n_features,
                "layer_name": GlobalState.layer_name,
                "allow_audio_upload": GlobalState.allow_audio_upload,
            }
        )
    else:
...
def get_top_files():
    args = {
        ...
        "max_val": (
            float(request.args.get("max_val")) if request.args.get("max_val") else None
        ),
        "min_val": (
            float(request.args.get("min_val")) if request.args.get("min_val") else None
        ),
...
def process_uploaded_audio(request):
    ...
    if sr != SAMPLE_RATE:
        ...
        audio = np.interp(
            np.linspace(0, len(audio) - 1, resampled_len), np.arange(len(audio)), audio
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, required=True, help="Path to feature configuration file"
    )
    parser.add_argument(
        "--from_disk", action="store_true", help="Whether to load activations from disk"
    )
    parser.add_argument(
        "--files_to_search",
        type=int,
    ...
    init_gui_data(
        args.config, args.from_disk, args.files_to_search, args.no_audio_upload
    )
